...
namespace SnakeBite
{
    internal static class ModManager
    {
        ...
        public static bool InstallMod(List<string> ModFiles, bool skipCleanup = false)
        {
        ...
        public static bool UninstallMod(CheckedListBox.CheckedIndexCollection modIndices)
        {
            SettingsManager manager = new SettingsManager(GameDir);
        ...
        public static List<string> FilterModValidity(List<string> ModFiles)
        {
            // check if mods are too old for snakebite, snakebite is too old for mod, and whether mod was for older version of game.
            // remove from list if mod is too old for snakebite or snakebite is too old for mod. ask user for input if mod was for older version of the game.
        ...
        public static List<string> FilterModConflicts(List<string> ModFiles)
        {
            //TODO check if mods in list conflict with installed mods, with game files, or with eachother.
            // ask user for input if a conflict is found.
            ...
            for (int i = ModFiles.Count() - 1; i >= 0; i--)
            {
                ...
                foreach (ModEntry mod in mods) // iterate through installed mods [Morbid: TODO iterate pftxs files from new mod]
                {
                    foreach (ModFileEntry fileEntry in metaData.ModFileEntries) // iterate external files from new mod
                    {
                        ModFileEntry conflicts = mod.ModFileEntries.FirstOrDefault(entry => Tools.CompareHashes(entry.FilePath, fileEntry.FilePath));
